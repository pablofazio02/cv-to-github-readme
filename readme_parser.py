...
def extract_data_from_pdf(path):
    ...
    _COMBINED_BADGE_KEYS = list(LANG_BADGES.keys()) + list(FRAMEWORK_BADGES.keys()) + list(DB_BADGES.keys()) + list(DEVELOPMENT_TOOLS_BADGES.keys())

    def detect_badge_keys_from_text(text):
        found = []
        if not text:
            return found
        t = text.lower()
        tokens = re.findall(r"[A-Za-z0-9\+\#\.\-]{2,}", t)
    ...
    skills = detect_badge_keys_from_text(text)
...
def generate_readme(data):
    ...
    if github_username:
        ...
        # render visual cards using github-readme-stats pin images if we have repos
        if top_repos:
            md.append('<p align="center">')
            ...
            # fallback: per-language visual badges/search links
            if 'langs_found' in locals() and langs_found:
                md.append('<p align="center">')
                for label, badge in langs_found:
                    lang_q = urllib.parse.quote(label)
